...
void main() {
  group('Dijkstra:', () {
    test('shortestPath(s)', () {
      ...
      expect(allPaths[a]?.prevVertex?.data, isNull);
      expect(allPaths[b]?.distance, 6.0);
      expect(allPaths[b]?.prevVertex?.data, 'E');
      expect(allPaths[c]?.distance, 4.0);
      expect(allPaths[c]?.prevVertex?.data, 'G');
      expect(allPaths[d]?.distance, 7.0);
      expect(allPaths[d]?.prevVertex?.data, 'E');
      expect(allPaths[e]?.distance, 5.0);
      expect(allPaths[e]?.prevVertex?.data, 'C');
      expect(allPaths[f]?.distance, 9.0);
      expect(allPaths[f]?.prevVertex?.data, 'A');
      expect(allPaths[g]?.distance, 1.0);
      expect(allPaths[g]?.prevVertex?.data, 'A');
      expect(allPaths[h]?.distance, isNull);
      expect(allPaths[h]?.prevVertex?.data, isNull);
